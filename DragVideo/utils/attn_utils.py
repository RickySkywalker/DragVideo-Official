...
class MutualSelfAttentionControl(AttentionBase):
    ...
    def __init__(self, start_step=4, start_layer=10, layer_idx=None, step_idx=None, total_steps=50, guidance_scale=7.5,drag_batch=3, ):
        """
    ...
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, **kwargs):
        ...
        if True:
            qu, qc = q[0 * self.drag_batch : 2 * self.drag_batch], q[2 * self.drag_batch : 4 * self.drag_batch]
            ku, kc = k[0 * self.drag_batch : 2 * self.drag_batch], k[2 * self.drag_batch : 4 * self.drag_batch]
            vu, vc = v[0 * self.drag_batch : 2 * self.drag_batch], v[2 * self.drag_batch : 4 * self.drag_batch]

            # merge queries of source and target branch into one so we can use torch API

            qu = torch.cat([qu[0 * self.drag_batch : 1 * self.drag_batch], qu[1 * self.drag_batch : 2 * self.drag_batch]], dim=2)
            qc = torch.cat([qc[0 * self.drag_batch : 1 * self.drag_batch], qc[1 * self.drag_batch: 2 * self.drag_batch]], dim=2)

            out_u = F.scaled_dot_product_attention(qu, ku[0 * self.drag_batch : 1 * self.drag_batch], vu[0 * self.drag_batch : 1 * self.drag_batch], attn_mask=None, dropout_p=0.0, is_causal=False)

            # print(f"before process out_u shape: {out_u.shape}")
            ...
            out_c = F.scaled_dot_product_attention(qc, kc[0 * self.drag_batch : 1 * self.drag_batch], vc[0 * self.drag_batch : 1 * self.drag_batch], attn_mask=None, dropout_p=0.0, is_causal=False)

            # print(f"before process out_c shape: {out_c.shape}")
            ...
            q = torch.cat([q[0 * self.drag_batch : 1 * self.drag_batch], q[1 * self.drag_batch: 2 * self.drag_batch]], dim=2)
            out = F.scaled_dot_product_attention(q, k[0 * self.drag_batch : 1 * self.drag_batch], v[0 * self.drag_batch : 1 * self.drag_batch], attn_mask=None, dropout_p=0.0, is_causal=False)
